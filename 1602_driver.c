 ...
 static void lcdWriteCmd(lcd_t *const lcd, unsigned char cmd, uint8_t lcd_opt)
 {
     ...
     if (lcd_opt == 1){
         ...
         gpio_set_level (lcd->regSel, GPIO_STATE_HIGH);
     }
 ...
 void lcdInit(lcd_t *const lcd)
 {
     ...
     /* Initialize LCD */
     lcdWriteCmd(lcd, 0x28, LCD_CMD); // 4-bit: DL=0, 2-line: N=1, 5x8: F=0 
     lcdWriteCmd(lcd, 0x08, LCD_CMD); // Instruction Flow was Display Off: D=0 cursor off C=0 blinking off B=0
     lcdWriteCmd(lcd, 0x01, LCD_CMD); // Clear LCD cursor home
     lcdWriteCmd(lcd, 0x06, LCD_CMD); // Auto-Increment I/D=1 S=0
     lcdWriteCmd(lcd, 0x0C, LCD_CMD); // Display On, No blink was 0c
 }
